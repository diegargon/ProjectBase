...
function Newspage_AdminOptions($news) {
    ...
    $content .= "<li><a href=''>{$LANGDATA['L_NEWS_APPROVED']}</a></li>";
    //$content .= "<li><a href=''>{$LANGDATA['L_NEWS_DISABLE']}</a></li>";
    $content .= "<li><a href='/newspage.php?nid={$news['nid']}&lang={$news['lang']}&news_delete={$news['nid']}&lang_id={$news['lang_id']}' onclick=\"return confirm('{$LANGDATA['L_NEWS_CONFIRM_DEL']}')\">{$LANGDATA['L_NEWS_DELETE']}</a></li>";
    $content .= "</ul>";
    $content .= "</nav>";
    $content .= "</div>";
