...
function news_new_page() {
    ...
    $form_data['limited_edit'] = 1;    
    $form_data['can_change_author'] = "disabled";
    $form_data['author'] = $user['username'];
    
    $tpl->addto_tplvar("NEWS_FORM_INPUT_HIDDEN", "<input type='hidden' name='nid' value='$nid'/><input type='hidden' name='news_lang' value='$lang'/><input type='hidden' name='num_pages' value='$num_pages'/>");
...
function news_newpage_submit_new($news_data) {
    ...
    $query = $db->select_all("news", array("nid" => $news_data['nid'], "lang_id" => $news_data['lang_id']), "ORDER BY page ASC");
    
    if( ($num_page = $db->num_rows($query)) <= 0) {
        return false; 
    }
