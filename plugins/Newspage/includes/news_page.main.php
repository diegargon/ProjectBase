...
function news_nav_options($news) { //TODO Use Template
    ...
    if ((defined('ACL') && $acl_auth->acl_ask("admin_all||news_admin")) || (!defined('ACL') && $user['isAdmin'] == 1)
        ...
        if ($news['featured'] == 1 && $news['page'] == 1) {
            $content .= "<li><a class='link_active' rel='nofollow' href='/{$config['CON_FILE']}?module=Newspage&page=news&nid={$news['nid']}&lang={$news['lang']}&news_featured=0&featured_value=0&lang_id={$news['lang_id']}&admin=1''>{$LANGDATA['L_NEWS_FEATURED']}</a></li>";
        } else if ($news['page'] == 1) {
        ...
        if ($news['moderation'] && $news['page'] == 1) {
            $content .= "<li><a rel='nofollow' href='/{$config['CON_FILE']}?module=Newspage&page=news&nid={$news['nid']}&lang={$news['lang']}&news_approved={$news['nid']}&lang_id={$news['lang_id']}&admin=1'>{$LANGDATA['L_NEWS_APPROVED']}</a></li>";
        }
        ...
        if ($config['NEWS_SELECTED_FRONTPAGE'] && $news['page'] == 1) {
            if ($news['frontpage'] == 1) {
                ...
                $content .= "<li><a rel='nofollow' href='/{$config['CON_FILE']}?module=Newspage&page=news&nid={$news['nid']}&lang={$news['lang']}&news_frontpage=1&lang_id={$news['lang_id']}'>{$LANGDATA['L_NEWS_FRONTPAGE']}</a></li>";
            }
        }
    }
    return $content;
