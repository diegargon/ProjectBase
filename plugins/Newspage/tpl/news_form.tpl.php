                ...
                <div class="news_submit_center_wrapper">
                    ...
                    <div class="submit_items">
                        <p>
                            ...
                            <input value="<?= isset($data['title']) ? $data['title'] : null ?>"  minlength="<?= $cfg['NEWS_TITLE_MIN_LENGHT'] ?>" maxlength="<?= $cfg['NEWS_TITLE_MAX_LENGHT'] ?>" id="news_title" name="news_title" required="required" type="text" placeholder=""/>
                        </p>
                    </div>
                    <div class="submit_items">
                        <label for="news_lead"><?= $LNG['L_NEWS_LEAD'] ?> </label>
                        <textarea required="required"  minlength="<?= $cfg['NEWS_LEAD_MIN_LENGHT'] ?>" maxlength="<?= $cfg['NEWS_LEAD_MAX_LENGHT'] ?>" id="news_lead" name="news_lead" ><?= isset($data['lead']) ? $data['lead'] : null ?></textarea>
                    </div>
                    <div class="submit_items">
                        <label for="news_text"><?= $LNG['L_NEWS_TEXT'] ?> </label>
                        <?= !empty($data['news_text_bar']) ? $data['news_text_bar'] : null ?>
                        <textarea required="required"  minlength="<?= $cfg['NEWS_TEXT_MIN_LENGHT'] ?>" maxlength="<?= $cfg['NEWS_TEXT_MAX_LENGHT'] ?>" id="news_text" name="news_text" ><?= isset($data['text']) ? $data['text'] : null ?></textarea>
                        <div id="EditorBtnBottomContainer">
                            <input class="btnPreview" type='button' id="btnShowPreview" value="<?= $LNG['L_NEWS_PREVIEW'] ?>"/>
                            <input class="btnPreview" type='button' id="btnHiddePreview" value="<?= $LNG['L_NEWS_HIDDE_PREVIEW'] ?>"/>
                        </div>
                        <div id="preview"></div>
                    </div>
                    <?= !empty($tpldata['NEWS_FORM_MIDDLE_OPTION']) ? $tpldata['NEWS_FORM_MIDDLE_OPTION'] : null ?>
                    
                    <?php if ($cfg['NEWS_SOURCE'] && !empty($data['news_auth']) && $data['news_auth'] != "translator") { ?>
                        <div class="submit_items">
                            <p> 
                                <label for="news_source"><?= $LNG['L_NEWS_SOURCE'] ?> </label>
                                <input  value="<?= isset($data['news_source']) ? $data['news_source'] : null ?>"  minlength="<?= $cfg['NEWS_LINK_MIN_LENGHT'] ?>" maxlength="<?= $cfg['NEWS_LINK_MAX_LENGHT'] ?>" id="news_source" class="news_link" name="news_source" type="text" placeholder="http://site.com"/>
                            </p>
                        </div>
                    <?php } ?>
                    <?php if ($cfg['NEWS_RELATED'] && !empty($data['news_auth']) && $data['news_auth'] != "translator") { ?>
                        <div class="submit_items">
                            <p>
                                <label for="news_new_related"><?= $LNG['L_NEWS_RELATED'] ?> </label>
                                <input  value="<?= isset($data['news_new_related']) ? $data['news_new_related'] : null ?>"  minlength="<?= $cfg['NEWS_LINK_MIN_LENGHT'] ?>" maxlength="<?= $cfg['NEWS_LINK_MAX_LENGHT'] ?>" id="news_new_related" class="news_link" name="news_new_related" type="text" placeholder="http://site.com"/>
                    ...
                    <div class="submit_buttom">
                        <p>
                            <input type="submit" id="newsFormSubmit" name="newsFormSubmit" class="btnSubmitForm" value="<?= $LNG['L_CREATE_NEWS'] ?>" />
                        </p>
                    </div>
                </div>
            </section>
